...
struct patient
{
    ...
    int id, age, doc, tkn1, shift,bedNumber;
    char time1[30];
    float bill;
    float discount;
    float final_bill;
...
struct Appointment {
    int id;            // patient id
    int shift;         // 1 = morning, 2 = evening
    struct Appointment* next;
};

struct Appointment* head = NULL;  // global head pointer for appointment list


// Utility to check if any appointment exists for a shift
int hasShiftAppointments(int shift) {
...
// Delete one appointment from given shift
void deleteOneAppointmentFromShift(int shift) {
    struct Appointment* temp = head;
    ...
    while (temp != NULL) {
        if (temp->shift == shift) {
            ...
            // Example bed stack operation: Free bed number associated with patient id
            if (bedStack.top < MAX_BEDS - 1) {
                bedStack.beds[++bedStack.top] = temp->id;
            }

            printf("\n\n");
            space printf("PROCESSING APPOINTMENT COMPLETION\n");
            printf("\n\n");
            space printf("Deleting appointment with patient ID: %d from %s shift\n", temp->id, shift == 1 ? "MORNING" : "EVENING");
...
void deleteAppointment() {
    if (hasShiftAppointments(1)) {
        // Delete one morning shift appointment first
        deleteOneAppointmentFromShift(1);
    } else if (hasShiftAppointments(2)) {
        // If no morning shift left, delete one evening shift appointment
        deleteOneAppointmentFromShift(2);
    } else {
        printf("\n\n");
        space printf("No appointments left to delete.\n");
    }
}
